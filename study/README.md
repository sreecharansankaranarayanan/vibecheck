...
2. [Participant Criteria & Recruitment](#2-participant-criteria--recruitment)
...
6. [Grading & Data Collection](#6-grading--data-collection)
...
- [ ] Verify they can run `npm install && npm run dev` → login screen appears
...
3. Group C: copy the telemetry JSONL to your data folder (see §3 above)
...
- At the end: ask them to demonstrate: enroll a course → hard refresh → course should still be there
- **Pass:** Bug is fixed (enrolled courses persist). Record time-to-fix.
- **Fail:** Bug not fixed in 30 minutes. Record what they tried.

### Session end

1. Administer the post-session survey (§7)
...
**What to build** (listed in the app's README.md):
...
print(f'Phase 2 chi-square: χ²={chi2:.2f}, df={dof}, p={p:.4f}')

# Paper result: χ²(2)=13.8, p=.001
```
...
events = []
for f in glob.glob('data/telemetry/*.jsonl'):
...
# Attempts per gate encounter
gate_shown = df_tel[df_tel.event == 'gate_shown']
gate_passed = df_tel[df_tel.event == 'gate_passed']
# Median attempts: paper result = 2.4 (SD=1.1)
attempts = df_tel.groupby('sessionId').apply(
    lambda g: g[g.event.isin(['gate_shown'])].shape[0]
)
...
| Median gate attempts | — | — | 2.4 (SD=1.1) |

**Key finding:** Groups B and C had near-identical productivity in Phase 1, but Group C had a 2.7× higher repair success rate than Group B in Phase 2 (61.5% vs 23.1%, χ²(2)=13.8, p=.001).
