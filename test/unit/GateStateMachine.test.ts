...
import { transition, makeInitialContext } from '../../src/gate/GateStateMachine';
...
describe('GateStateMachine', () => {
  ...
  it('IDLE → WAITING_FOR_EXPLANATION on GATE_TRIGGERED', () => {
    const ctx = makeInitialContext();
    const next = transition(ctx, { type: 'GATE_TRIGGERED', codeSnippet: 'const x = 1;' });
  ...
  it('WAITING → JUDGING on EXPLANATION_SUBMITTED, increments attempt count', () => {
    ...
    const next = transition(ctx, { type: 'EXPLANATION_SUBMITTED', explanation: 'it assigns 1 to x' });
    expect(next.state).toBe('JUDGING');
    expect(next.attemptCount).toBe(1);
  });

  it('JUDGING → PASS on JUDGE_PASS', () => {
    const ctx: GateContext = { state: 'JUDGING', codeSnippet: 'x', attemptCount: 1 };
  ...
  it('FAIL → WAITING on GATE_TRIGGERED (retry), preserves attempt count', () => {
    const ctx: GateContext = { state: 'FAIL', codeSnippet: 'x', attemptCount: 2 };
  ...
  it('PASS → IDLE on RESET', () => {
    const ctx: GateContext = { state: 'PASS', codeSnippet: 'x', attemptCount: 1 };
  ...
  it('throws on invalid transition', () => {
    const ctx = makeInitialContext();
    expect(() => transition(ctx, { type: 'EXPLANATION_SUBMITTED', explanation: 'hi' })).toThrow();
  ...
  it('does not mutate the original context', () => {
    ...
    const next = transition(original, { type: 'GATE_TRIGGERED', codeSnippet: 'test' });
