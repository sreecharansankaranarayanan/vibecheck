...
describe('Modal prompt text (paper §3.2)', () => {
  ...
  it('contains the exact modal prompt from the paper', () => {
    ...
    expect(html).toContain('Before applying this code, explain its causal logic.');
...
describe('Default configuration values (paper §3)', () => {
  ...
  it('default judgeTemperature is 0.1 (paper §3.3 — deterministic grading)', () => {
    const pkg = JSON.parse(readFileSync(join(ROOT, 'package.json'), 'utf8')) as {
      contributes: { configuration: { properties: { 'vibecheck.judgeTemperature': { default: number } } } };
    };
    expect(pkg.contributes.configuration.properties['vibecheck.judgeTemperature'].default).toBe(0.1);
  });

  it('default passThreshold is 3 (paper §3.3 — Relational level)', () => {
    const pkg = JSON.parse(readFileSync(join(ROOT, 'package.json'), 'utf8')) as {
      contributes: { configuration: { properties: { 'vibecheck.passThreshold': { default: number } } } };
    };
    expect(pkg.contributes.configuration.properties['vibecheck.passThreshold'].default).toBe(3);
  });
});
